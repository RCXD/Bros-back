...
def get_config_paths(app):
    """앱 설정에서 경로 가져오기"""
    return {
        'dummy_profile_dir': Path(app.config.get('DUMMY_PROFILE_IMG_DIR', r"\\192.168.1.89\share\dummy data\profile_images")),
        ...
        if api_result:
            ...
            if '업데이트' in response_message or '수정' in response_message:
                ...
                if (idx + 1) % 100 == 0:
                    ...
                    if verified:
                        log.debug(f"    ✅ 프로필 이미지 조회 성공: user_id={user.user_id}")
                    else:
                        log.warning(f"    ❌ 프로필 이미지 조회 실패: user_id={user.user_id}")
            else:
                total_failed += 1
...
def _generate_images_direct(app, dummy_image_dir, image_storage_dir):
    ...
    for category_name, post_list in posts_by_category.items():
        ...
        for post in post_list:
            num_images = random.choice([0, 0, 1, 1, 1, 2, 2, 2, 2, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 3])
            ...
            if not user:
                log.debug(f"Post {post.post_id}의 작성자를 찾을 수 없습니다. 건너뜁니다.")
                continue
    ...
    log.info(f"이미지가 있는 게시글: {db.session.query(Post).join(Image).distinct().count()}개")
    log.info("="*60)
...
def _generate_images_via_api(app, dummy_image_dir):
    ...
    for category_name, post_list in posts_by_category.items():
        ...
        for post in post_list:
            ...
            log.debug(f"Post {post.post_id} ({user.nickname}): {len(image_paths)}개 이미지 업로드 시도...")
                ...
                if '수정' in response_message or '완료' in response_message or 'uploaded' in response_message.lower():
                    ...
                    if upload_counter >= 100 and uploaded_image_info:
                        ...
                        if last_img['uuid']:
                            ...
                            if verified:
                                log.debug(f"이미지 조회 성공: post_id={last_img['post_id']}, uuid={last_img['uuid']}")
                            else:
                                log.warning(f"이미지 조회 실패: post_id={last_img['post_id']}, uuid={last_img['uuid']}")
                        upload_counter = 0
                else:
        ...
        for img_info in sample_images:
            if img_info['uuid']:
                if uploader.verify_post_image(img_info['uuid']):
                    ...
                    failed_verify_count += 1
                    log.warning(f"검증 실패: post_id={img_info['post_id']}, uuid={img_info['uuid']}")
