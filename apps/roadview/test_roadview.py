...
def test_api_keys():
    ...
    print(f"GOOGLE_MAPS_API_KEY: {'✓ SET' if google_key and google_key != 'your-google-api-key-here' else '✗ NOT SET'}")
    if google_key and google_key != 'your-google-api-key-here':
        print(f"  Value: {google_key[:10]}...{google_key[-4:]}")
    
    print(f"KAKAO_REST_API_KEY: {'✓ SET' if kakao_key and kakao_key != 'your-kakao-api-key-here' else '✗ NOT SET'}")
    if kakao_key and kakao_key != 'your-kakao-api-key-here':
        print(f"  Value: {kakao_key[:10]}...{kakao_key[-4:]}")
    
    print(f"NAVER_CLIENT_ID: {'✓ SET' if naver_id and naver_id != 'your-naver-client-id-here' else '✗ NOT SET'}")
    if naver_id and naver_id != 'your-naver-client-id-here':
        print(f"  Value: {naver_id[:10]}...{naver_id[-4:]}")
    
    print(f"NAVER_CLIENT_SECRET: {'✓ SET' if naver_secret and naver_secret != 'your-naver-client-secret-here' else '✗ NOT SET'}")
...
def test_roadview_loader():
    ...
    print(f"Google Street View client: {'✓ Available' if loader.google else '✗ NOT AVAILABLE'}")
    print(f"Kakao Roadview client: {'✓ Available' if loader.kakao else '✗ NOT AVAILABLE'}")
    print(f"Naver Street View client: {'✓ Available' if loader.naver else '✗ NOT AVAILABLE'}")
