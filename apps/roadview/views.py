...
def check_roadview():
    ...
    rv_request = RoadviewRequest(
        ...
        latitude=latitude,
        longitude=longitude,
        preferred_provider=RoadviewProvider[preferred_provider.upper().replace(' ', '_')] if preferred_provider else None
    )
    ...
    try:
        ...
        result = loader.load_best_roadview(latitude, longitude, radius, preferred_provider)
        ...
        if result['success']:
            ...
            provider_enum = RoadviewProvider[result['provider'].upper().replace(' ', '_') + '_STREET_VIEW'] \
                if result['provider'] == 'google' else \
                RoadviewProvider[f"{result['provider'].upper()}_{'ROADVIEW' if result['provider'] == 'kakao' else 'STREET_VIEW'}"]
            ...
            if metadata.get('date'):
                try:
                    ...
                    if len(date_str) == 7:  # YYYY-MM format
                        roadview.image_date = datetime.strptime(date_str, '%Y-%m').date()
                except:
                    pass
...
def get_user_history():
    ...
    requests_query = RoadviewRequest.query.filter_by(user_id=user_id) \
        .order_by(RoadviewRequest.created_at.desc())
...
def update_api_usage(db, provider: RoadviewProvider, success: bool = True, cached: bool = False):
