    ...
    def attach_model(self) -> Dict:
        ...
        try:
            response = requests.get(
                f"{self.server_url}/attach-model", timeout=self.timeout
            )
    ...
    def detach_model(self) -> Dict:
        ...
        try:
            response = requests.delete(
                f"{self.server_url}/detach-model", timeout=self.timeout
            )
            response.raise_for_status()
            self.model_attached = False
            return response.json()
    ...
    def detect_objects(self, image_data: bytes, confidence: float = 0.5) -> Dict:
        ...
        try:
            ...
            response = requests.post(
                f"{self.server_url}/detect", files=files, timeout=self.timeout
            )
            response.raise_for_status()
            ...
            try:
                ...
                return {
                    "error": f"Invalid JSON response from AI server: {str(json_err)}",
                    "status": "failed",
...
class DetectionResultProcessor:
    ...
    def process_segmentation_results(seg_output: Dict) -> Tuple[List[Dict], Dict]:
        ...
        for segment in seg_output.get("segments", []):
            seg = {
                ...
                "percentage": (
                    (segment.get("pixel_count", 0) / total_pixels * 100)
                    if total_pixels > 0
                    else 0
                ),
    ...
    @staticmethod
    def normalize_bbox(
        bbox: Dict, img_width: int, img_height: int, is_normalized: bool = False
    ) -> Dict:
        """
...
# Singleton cache for AI server clients
_AI_CLIENT_CACHE: Dict[str, AIServerClient] = {}


def get_ai_server_client(detection_type: str = "object") -> AIServerClient:
    """
    감지 유형에 따라 AI 서버 클라이언트 가져오기 (싱글톤 패턴)

    클라이언트 인스턴스를 캐싱하여 attach_model()이 최초 한 번만 호출되도록 함.
    ...
    Returns:
        AIServerClient 인스턴스 (캐싱됨)
    """
    global _AI_CLIENT_CACHE

    # 캐시에 이미 있으면 반환
    if detection_type in _AI_CLIENT_CACHE:
        return _AI_CLIENT_CACHE[detection_type]
    ...
    client = AIServerClient(server_url)

    # 캐시에 저장
    _AI_CLIENT_CACHE[detection_type] = client

    return client


def validate_image(
    image_data: bytes, max_size_mb: int = 10
    ...
    if size_mb > max_size_mb:
        return (
            False,
            f"이미지 크기 ({size_mb:.2f}MB)가 제한 ({max_size_mb}MB)을 초과했습니다",
        )
