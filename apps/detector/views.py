...
def api_info():
    ...
    info = {
        "module": "detector",
        "base_path": "/detector",
        "description": "AI 객체 탐지 및 분석",
        "endpoints": [
            {
                "path": "/detector/detect",
                "method": "POST",
                "auth_required": True,
                "description": "이미지 객체 탐지",
                "form_data": {
                    "image": "이미지 파일 (필수)",
                    "detection_type": "탐지 타입 (선택)",
                },
            },
            {
                "path": "/detector/result/<detection_id>",
                "method": "GET",
                "auth_required": True,
                "description": "탐지 결과 조회",
            },
            {
                "path": "/detector/history",
                "method": "GET",
                "auth_required": True,
                "description": "탐지 히스토리 조회",
            },
            {
                "path": "/detector/api_info",
                "method": "GET",
                "auth_required": False,
...
def detect_objects():
    ...
    try:
        ...
        for obj_data in objects_list:
            obj = DetectedObject(detection_id=detection.detection_id, **obj_data)
            db.session.add(obj)
...
def semantic_segmentation():
    ...
    try:
        ...
        for seg_data in segments_list:
            segment = SemanticSegment(detection_id=detection.detection_id, **seg_data)
    ...
    try:
        ...
        if not ai_client.health_check():
            ...
            detection.error_message = (
                "Road segmentation server (8889) is currently unavailable"
            )
            db.session.commit()
            return (
                jsonify(
