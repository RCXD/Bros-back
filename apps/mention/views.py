...
def serialize_mention(mention):
    ...
    return {
        ...
        "mentioned_username": mention.mentioned_user.username if mention.mentioned_user else None,
...
def create_mention():
    ...
    if (post_id and reply_id) or (not post_id and not reply_id):
        return jsonify({"error": "정확히 하나의 대상(post_id or reply_id)만 지정해야 합니다"}), 400
    ...
    try:
        ...
        return jsonify({
            "message": "멘션 생성 완료",
            "mention": serialize_mention(new_mention)
        }), 201
...
def get_my_mentions():
    ...
    return jsonify({
        "mentions": [serialize_mention(m) for m in mentions]
    }), 200
...
def get_sent_mentions():
    ...
    mentions = Mention.query.filter_by(mentioner_id=current_user_id)\
        .order_by(Mention.created_at.desc()).all()
...
def get_post_mentions(post_id):
    ...
    mentions = Mention.query.filter_by(post_id=post_id)\
        .order_by(Mention.created_at.desc()).all()
