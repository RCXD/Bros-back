...
def extract_images_from_docx(
    ...
    Returns:
        tuple[List[str], Optional[str]]: (모든 이미지 식별자 리스트, 1페이지 이미지 식별자)
    """
    doc = Document(doc_path)
    image_identifiers = []
    first_page_image = None

    # 문서 요소를 순회하며 이미지 추출 (순서 보장)
    image_counter = 1
    image_rels = {}

    # relationship에서 이미지 정보 수집
    ...
    for element in doc.element.body:
        ...
        if isinstance(element, CT_P):
            ...
            for run in para.runs:
                # drawing 또는 inline shape 찾기
                drawings = run.element.findall(
                    ".//{http://schemas.openxmlformats.org/wordprocessingml/2006/main}drawing",
                    run.element.nsmap,
                )
                for drawing in drawings:
                    # blip 요소에서 relationship ID 찾기
                    ...
                    for blip in blips:
                        ...
                        if embed_id and embed_id in image_rels:
                            rel = image_rels[embed_id]
                            try:
                                image_data = rel.target_part.blob
                                ext = Path(rel.target_ref).suffix or ".png"

                                # 파일명: product_code_숫자.확장자
                                ...
                                # 첫 번째 이미지를 1페이지 이미지로 간주
                                if image_counter == 1:
                                    first_page_image = image_identifier
                                ...
                                for blip in blips:
                                    ...
                                    if embed_id and embed_id in image_rels:
                                        ...
                                        try:
                                            ...
                                            image_filename = (
                                                f"{product_code}_{image_counter}{ext}"
                                            )
                                            image_identifier = (
                                                f"{product_code}_{image_counter}"
                                            )
                                            image_path = output_dir / image_filename
                                            ...
                                            print(
                                                f"이미지 추출 실패 ({rel.target_ref}): {e}"
                                            )
                                            continue

    return image_identifiers, first_page_image
...
def create_product_from_docx(
    ...
    # 이미지 추출 (문서 순서대로)
    image_identifiers, first_page_image = extract_images_from_docx(
        doc_path, output_image_dir, product_code
    )
    ...
    # Product 객체 생성
    # product_img: 1페이지 이미지 (상품 정보 이미지)
    # product_detail_img: 나머지 모든 이미지 (1페이지 이미지 제외)
    detail_images = [img for img in image_identifiers if img != first_page_image]
    ...
    product = Product(
        ...
        name=product_name,
        category=category,
        product_img=first_page_image,  # 1페이지 이미지를 메인으로
        product_detail_img=detail_images,  # 나머지를 상세 이미지로
    )
