...
def create_report():
    ...
    if not all([report_target_type, report_target_id, report_reason]):
        return (
            jsonify(
                {"message": "report_target_type, report_target_id, report_reason 필수"}
            ),
            400,
        )
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "report_target_type는 USER, POST, REPLY만 가능합니다."}
            ),
            400,
        )
...
def get_my_reports():
    ...
    current_user_id = get_jwt_identity()
    reports_list = Report.query.filter_by(user_id=current_user_id).paginate(
        page=page, per_page=10
    )
    result = []
    for r in reports_list.items:
        result.append(
            {
